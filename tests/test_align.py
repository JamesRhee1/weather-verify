...
from src.schema import SOURCE_GROUND_TRUTH, SOURCE_OPENMETEO_ECMWF, STANDARD_COLUMNS
...
def test_align_joins_forecast_with_ground_truth():
    truth = pd.DataFrame(
        [
            _row("2026-06-01T12:00", 0, 20.0, SOURCE_GROUND_TRUTH),
            _row("2026-06-01T13:00", 0, 21.0, SOURCE_GROUND_TRUTH),
        ]
    )
    fcst = pd.DataFrame(
        [
...
def test_align_skips_mismatched_valid_time():
    truth = pd.DataFrame([_row("2026-06-01T12:00", 0, 20.0, SOURCE_GROUND_TRUTH)])
...
def test_align_drops_rows_with_missing_values():
    truth = pd.DataFrame(
        [
            _row("2026-06-01T12:00", 0, 20.0, SOURCE_GROUND_TRUTH),
            _row("2026-06-01T13:00", 0, 20.0, SOURCE_GROUND_TRUTH),
        ]
    )
    fcst = pd.DataFrame(
        [
